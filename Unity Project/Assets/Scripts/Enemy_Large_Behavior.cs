...
//behavior for larger enemies that spawn regular shooting enemies
public class Enemy_Large_Behavior : MonoBehaviour {
	...
	public int spawnTimer = 400;
	public int nextSpawn = 400;
	//prefab to spawn (should be basic shooting enemy)
	...
	void Update () {
		...
		if(spawnTimer == 0 && (target == transform || targetDistance > 7)){ 
			spawnTimer += nextSpawn;
			Instantiate(spawnPrefab, new Vector3(transform.position.x, transform.position.y + 3, transform.position.z + 4), Quaternion.identity);
		}
